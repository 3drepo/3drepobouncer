...
OdSharedPtr<OdGiConveyorGeometryDumper> OdGiConveyorGeometryDumper::createObject(OdGiDumper* m_pDumper, GeometryCollector* collector)
{
    ...
    const OdGiVertexData* pVertexData)
{
    ...
    for (OdInt32 row = 0; row < rows; row++)
    {
        for (OdInt32 col = 0; col < columns; col++)
        {
            ...
            pVt++;
            verts.push_back(repo::lib::RepoVector3D64(pV->x, pV->y, pV->z));
            //geometryCollector->addFace(verts);
    ...
    const OdGiVertexData* pVertexData)
{
    ...
    geometryCollector->stopMeshEntry();
    geometryCollector->startMeshEntry();
    repo_material_t mat; //TODO: materials will be included in the next commit
    mat.ambient = { 0,0.5,0 };
    mat.diffuse = { 1,0,0 };
    mat.emissive = { 0,1,0 };
    mat.specular = { 0,1,0 };
