...
constexpr double Absolute(double a) {
    return a >= 0.0 ? a : -a;
}
...
/* Constants set per machine by initexact() */
...
struct ops {
    ...
    inline void Two_Product_2Presplit(double& a, double& ahi, double& alo, double& b, double& bhi, double& blo, double& x, double& y) {
        x = (a * b);
        err1 = x - (ahi * bhi);
        err2 = err1 - (alo * bhi);
        err3 = err2 - (ahi * blo);
    ...
    inline void Square_Tail(double& a, double& x, double& y) {
        Split(a, ahi, alo);
        err1 = x - (ahi * ahi);
    ...
    inline void Two_One_Sum(double& a1, double& a0, double& b, double& x2, double& x1, double& x0) {
        Two_Sum(a0, b, _i, x0);
        Two_Sum(a1, _i, x2, x1);
    }
...
int grow_expansion(int elen, double* e, double b, double* h)                /* e and h can be the same. */
{
    ...
    for (eindex = 0; eindex < elen; eindex++) {
        enow = e[eindex];
        ops.Two_Sum(Q, enow, Qnew, h[eindex]);
        Q = Qnew;
    }
    h[eindex] = Q;
    return eindex + 1;
...
/* e and h can be the same, but f and h cannot. */
{
    ...
    for (hindex = 0; hindex < elen; hindex++) {
        hnow = e[hindex];
        ops.Two_Sum(Q, hnow, Qnew, h[hindex]);
        Q = Qnew;
    }
    h[hindex] = Q;
    hlast = hindex;
...
int fast_expansion_sum(int elen, double*  e, int flen, double*  f, double*  h)           /* h cannot be e or f. */
{
    ...
    if ((eindex < elen) && (findex < flen)) {
        if ((fnow > enow) == (fnow > -enow)) {
            ops.Fast_Two_Sum(enow, Q, Qnew, h[0]);
            enow = e[++eindex];
        }
        else {
            ops.Fast_Two_Sum(fnow, Q, Qnew, h[0]);
            fnow = f[++findex];
        }
        Q = Qnew;
        hindex = 1;
        while ((eindex < elen) && (findex < flen)) {
            if ((fnow > enow) == (fnow > -enow)) {
                ops.Two_Sum(Q, enow, Qnew, h[hindex]);
                enow = e[++eindex];
            }
            else {
                ops.Two_Sum(Q, fnow, Qnew, h[hindex]);
                fnow = f[++findex];
            }
            Q = Qnew;
            hindex++;
...
int fast_expansion_sum_zeroelim(int elen, double*  e, int flen, double*  f, double*  h)  /* h cannot be e or f. */
{
    ...
    if ((eindex < elen) && (findex < flen)) {
        if ((fnow > enow) == (fnow > -enow)) {
            ops.Fast_Two_Sum(enow, Q, Qnew, hh);
            enow = e[++eindex];
        }
        else {
            ops.Fast_Two_Sum(fnow, Q, Qnew, hh);
            fnow = f[++findex];
        }
        Q = Qnew;
        if (hh != 0.0) {
...
int linear_expansion_sum(int elen, double*  e, int flen, double*  f, double* h)         /* h cannot be e or f. */
{
    ...
    if ((eindex < elen) && ((findex >= flen)
        || ((fnow > enow) == (fnow > -enow)))) {
        ops.Fast_Two_Sum(enow, g0, Qnew, q);
        enow = e[++eindex];
    }
    else {
        ops.Fast_Two_Sum(fnow, g0, Qnew, q);
        fnow = f[++findex];
    }
    Q = Qnew;
    for (hindex = 0; hindex < elen + flen - 2; hindex++) {
        if ((eindex < elen) && ((findex >= flen)
            || ((fnow > enow) == (fnow > -enow)))) {
            ops.Fast_Two_Sum(enow, q, R, h[hindex]);
            enow = e[++eindex];
        }
        else {
            ops.Fast_Two_Sum(fnow, q, R, h[hindex]);
            fnow = f[++findex];
        }
        ops.Two_Sum(Q, R, Qnew, q);
        Q = Qnew;
    }
    h[hindex] = q;
    h[hindex + 1] = Q;
...
int linear_expansion_sum_zeroelim(int elen, double*  e, int flen, double*  f, double*  h)/* h cannot be e or f. */
{
    ...
    for (count = 2; count < elen + flen; count++) {
        if ((eindex < elen) && ((findex >= flen)
            || ((fnow > enow) == (fnow > -enow)))) {
            ops.Fast_Two_Sum(enow, q, R, hh);
            enow = e[++eindex];
        }
        else {
            ops.Fast_Two_Sum(fnow, q, R, hh);
            fnow = f[++findex];
        }
        ops.Two_Sum(Q, R, Qnew, q);
        Q = Qnew;
...
int scale_expansion(int elen, double*  e, double b, double*  h)            /* e and h cannot be the same. */
{
    ...
    ops.Split(b, bhi, blo);
    ops.Two_Product_Presplit(e[0], b, bhi, blo, Q, h[0]);
    hindex = 1;
    for (eindex = 1; eindex < elen; eindex++) {
        enow = e[eindex];
        ops.Two_Product_Presplit(enow, b, bhi, blo, product1, product0);
        ops.Two_Sum(Q, product0, sum, h[hindex]);
        hindex++;
        ops.Two_Sum(product1, sum, Q, h[hindex]);
        hindex++;
    }
    h[hindex] = Q;
    return elen + elen;
...
int scale_expansion_zeroelim(int elen, double*  e, double b, double*  h)   /* e and h cannot be the same. */
{
    ...
    ops.Split(b, bhi, blo);
    ops.Two_Product_Presplit(e[0], b, bhi, blo, Q, hh);
    hindex = 0;
    if (hh != 0) {
        h[hindex++] = hh;
    }
    for (eindex = 1; eindex < elen; eindex++) {
        enow = e[eindex];
        ops.Two_Product_Presplit(enow, b, bhi, blo, product1, product0);
        ops.Two_Sum(Q, product0, sum, hh);
        if (hh != 0) {
            h[hindex++] = hh;
        }
        ops.Fast_Two_Sum(product1, sum, Q, hh);
        if (hh != 0) {
            h[hindex++] = hh;
        }
    }
...
int compress(int elen, double*  e, double*  h)                         /* e and h may be the same. */
{
    ...
    for (eindex = elen - 2; eindex >= 0; eindex--) {
        enow = e[eindex];
        ops.Fast_Two_Sum(Q, enow, Qnew, q);
        if (q != 0) {
            h[bottom--] = Qnew;
            Q = q;
        }
    ...
    for (hindex = bottom + 1; hindex < elen; hindex++) {
        hnow = h[hindex];
        ops.Fast_Two_Sum(hnow, Q, Qnew, q);
        if (q != 0) {
            h[top++] = q;
        }
        Q = Qnew;
...
double orient2dadapt(double* pa, double* pb, double* pc, double detsum)
{
    ...
    ops.Two_Product(acx, bcy, detleft, detlefttail);
    ops.Two_Product(acy, bcx, detright, detrighttail);

    ops.Two_Two_Diff(detleft, detlefttail, detright, detrighttail, B3, B[2], B[1], B[0]);
    B[3] = B3;
    ...
    ops.Two_Diff_Tail(pa[0], pc[0], acx, acxtail);
    ops.Two_Diff_Tail(pb[0], pc[0], bcx, bcxtail);
    ops.Two_Diff_Tail(pa[1], pc[1], acy, acytail);
    ops.Two_Diff_Tail(pb[1], pc[1], bcy, bcytail);
    ...
    ops.Two_Product(acxtail, bcy, s1, s0);
    ops.Two_Product(acytail, bcx, t1, t0);
    ops.Two_Two_Diff(s1, s0, t1, t0, u3, u[2], u[1], u[0]);
    u[3] = u3;
    C1length = fast_expansion_sum_zeroelim(4, B, 4, u, C1);

    ops.Two_Product(acx, bcytail, s1, s0);
    ops.Two_Product(acy, bcxtail, t1, t0);
    ops.Two_Two_Diff(s1, s0, t1, t0, u3, u[2], u[1], u[0]);
    u[3] = u3;
    C2length = fast_expansion_sum_zeroelim(C1length, C1, 4, u, C2);

    ops.Two_Product(acxtail, bcytail, s1, s0);
    ops.Two_Product(acytail, bcxtail, t1, t0);
    ops.Two_Two_Diff(s1, s0, t1, t0, u3, u[2], u[1], u[0]);
    u[3] = u3;
...
double orient3dadapt(double* pa, double* pb, double* pc, double* pd, double permanent)
{
    ...
    ops.Two_Product(bdx, cdy, bdxcdy1, bdxcdy0);
    ops.Two_Product(cdx, bdy, cdxbdy1, cdxbdy0);
    ops.Two_Two_Diff(bdxcdy1, bdxcdy0, cdxbdy1, cdxbdy0, bc3, bc[2], bc[1], bc[0]);
    bc[3] = bc3;
    alen = scale_expansion_zeroelim(4, bc, adz, adet);

   ops.Two_Product(cdx, ady, cdxady1, cdxady0);
   ops.Two_Product(adx, cdy, adxcdy1, adxcdy0);
   ops.Two_Two_Diff(cdxady1, cdxady0, adxcdy1, adxcdy0, ca3, ca[2], ca[1], ca[0]);
    ca[3] = ca3;
    blen = scale_expansion_zeroelim(4, ca, bdz, bdet);

    ops.Two_Product(adx, bdy, adxbdy1, adxbdy0);
    ops.Two_Product(bdx, ady, bdxady1, bdxady0);
    ops.Two_Two_Diff(adxbdy1, adxbdy0, bdxady1, bdxady0, ab3, ab[2], ab[1], ab[0]);
    ab[3] = ab3;
   ...
   ops.Two_Diff_Tail(pa[0], pd[0], adx, adxtail);
   ops.Two_Diff_Tail(pb[0], pd[0], bdx, bdxtail);
   ops.Two_Diff_Tail(pc[0], pd[0], cdx, cdxtail);
   ops.Two_Diff_Tail(pa[1], pd[1], ady, adytail);
   ops.Two_Diff_Tail(pb[1], pd[1], bdy, bdytail);
   ops.Two_Diff_Tail(pc[1], pd[1], cdy, cdytail);
   ops.Two_Diff_Tail(pa[2], pd[2], adz, adztail);
   ops.Two_Diff_Tail(pb[2], pd[2], bdz, bdztail);
   ops.Two_Diff_Tail(pc[2], pd[2], cdz, cdztail);
    ...
    if (adxtail == 0.0) {
        if (adytail == 0.0) {
            ...
            negate = -adytail;
            ops.Two_Product(negate, bdx, at_blarge, at_b[0]);
            at_b[1] = at_blarge;
            at_blen = 2;
            ops.Two_Product(adytail, cdx, at_clarge, at_c[0]);
            at_c[1] = at_clarge;
            at_clen = 2;
        }
    }
        ...
        if (adytail == 0.0) {
            ops.Two_Product(adxtail, bdy, at_blarge, at_b[0]);
            at_b[1] = at_blarge;
            at_blen = 2;
            negate = -adxtail;
            ops.Two_Product(negate, cdy, at_clarge, at_c[0]);
            at_c[1] = at_clarge;
            at_clen = 2;
        }
        else {
            ops.Two_Product(adxtail, bdy, adxt_bdy1, adxt_bdy0);
            ops.Two_Product(adytail, bdx, adyt_bdx1, adyt_bdx0);
            ops.Two_Two_Diff(adxt_bdy1, adxt_bdy0, adyt_bdx1, adyt_bdx0,
                at_blarge, at_b[2], at_b[1], at_b[0]);
            at_b[3] = at_blarge;
            at_blen = 4;
            ops.Two_Product(adytail, cdx, adyt_cdx1, adyt_cdx0);
            ops.Two_Product(adxtail, cdy, adxt_cdy1, adxt_cdy0);
            ops.Two_Two_Diff(adyt_cdx1, adyt_cdx0, adxt_cdy1, adxt_cdy0,
    ...
    if (bdxtail == 0.0) {
        if (bdytail == 0.0) {
            ...
            negate = -bdytail;
            ops.Two_Product(negate, cdx, bt_clarge, bt_c[0]);
            bt_c[1] = bt_clarge;
            bt_clen = 2;
            ops.Two_Product(bdytail, adx, bt_alarge, bt_a[0]);
            bt_a[1] = bt_alarge;
            bt_alen = 2;
        }
    }
        ...
        if (bdytail == 0.0) {
            ops.Two_Product(bdxtail, cdy, bt_clarge, bt_c[0]);
            bt_c[1] = bt_clarge;
            bt_clen = 2;
            negate = -bdxtail;
            ops.Two_Product(negate, ady, bt_alarge, bt_a[0]);
            bt_a[1] = bt_alarge;
            bt_alen = 2;
        }
        else {
           ops.Two_Product(bdxtail, cdy, bdxt_cdy1, bdxt_cdy0);
           ops.Two_Product(bdytail, cdx, bdyt_cdx1, bdyt_cdx0);
           ops.Two_Two_Diff(bdxt_cdy1, bdxt_cdy0, bdyt_cdx1, bdyt_cdx0,
                bt_clarge, bt_c[2], bt_c[1], bt_c[0]);
            bt_c[3] = bt_clarge;
            bt_clen = 4;
            ops.Two_Product(bdytail, adx, bdyt_adx1, bdyt_adx0);
            ops.Two_Product(bdxtail, ady, bdxt_ady1, bdxt_ady0);
            ops.Two_Two_Diff(bdyt_adx1, bdyt_adx0, bdxt_ady1, bdxt_ady0,
    ...
    if (cdxtail == 0.0) {
        if (cdytail == 0.0) {
            ...
            negate = -cdytail;
            ops.Two_Product(negate, adx, ct_alarge, ct_a[0]);
            ct_a[1] = ct_alarge;
            ct_alen = 2;
            ops.Two_Product(cdytail, bdx, ct_blarge, ct_b[0]);
            ct_b[1] = ct_blarge;
            ct_blen = 2;
        }
    }
        ...
        if (cdytail == 0.0) {
            ops.Two_Product(cdxtail, ady, ct_alarge, ct_a[0]);
            ct_a[1] = ct_alarge;
            ct_alen = 2;
            negate = -cdxtail;
            ops.Two_Product(negate, bdy, ct_blarge, ct_b[0]);
            ct_b[1] = ct_blarge;
            ct_blen = 2;
        }
        else {
            ops.Two_Product(cdxtail, ady, cdxt_ady1, cdxt_ady0);
            ops.Two_Product(cdytail, adx, cdyt_adx1, cdyt_adx0);
            ops.Two_Two_Diff(cdxt_ady1, cdxt_ady0, cdyt_adx1, cdyt_adx0,
                ct_alarge, ct_a[2], ct_a[1], ct_a[0]);
            ct_a[3] = ct_alarge;
            ct_alen = 4;
            ops.Two_Product(cdytail, bdx, cdyt_bdx1, cdyt_bdx0);
            ops.Two_Product(cdxtail, bdy, cdxt_bdy1, cdxt_bdy0);
            ops.Two_Two_Diff(cdyt_bdx1, cdyt_bdx0, cdxt_bdy1, cdxt_bdy0,
    ...
    if (adxtail != 0.0) {
        if (bdytail != 0.0) {
            ops.Two_Product(adxtail, bdytail, adxt_bdyt1, adxt_bdyt0);
            ops.Two_One_Product(adxt_bdyt1, adxt_bdyt0, cdz, u3, u[2], u[1], u[0]);
            u[3] = u3;
            finlength = fast_expansion_sum_zeroelim(finlength, finnow, 4, u,
                finother);
            finswap = finnow; finnow = finother; finother = finswap;
            if (cdztail != 0.0) {
                ops.Two_One_Product(adxt_bdyt1, adxt_bdyt0, cdztail, u3, u[2], u[1], u[0]);
                u[3] = u3;
                finlength = fast_expansion_sum_zeroelim(finlength, finnow, 4, u,
                    finother);
                finswap = finnow; finnow = finother; finother = finswap;
        ...
        if (cdytail != 0.0) {
            negate = -adxtail;
            ops.Two_Product(negate, cdytail, adxt_cdyt1, adxt_cdyt0);
            ops.Two_One_Product(adxt_cdyt1, adxt_cdyt0, bdz, u3, u[2], u[1], u[0]);
            u[3] = u3;
            finlength = fast_expansion_sum_zeroelim(finlength, finnow, 4, u,
                finother);
            finswap = finnow; finnow = finother; finother = finswap;
            if (bdztail != 0.0) {
                ops.Two_One_Product(adxt_cdyt1, adxt_cdyt0, bdztail, u3, u[2], u[1], u[0]);
                u[3] = u3;
                finlength = fast_expansion_sum_zeroelim(finlength, finnow, 4, u,
                    finother);
                finswap = finnow; finnow = finother; finother = finswap;
    ...
    if (bdxtail != 0.0) {
        if (cdytail != 0.0) {
            ops.Two_Product(bdxtail, cdytail, bdxt_cdyt1, bdxt_cdyt0);
            ops.Two_One_Product(bdxt_cdyt1, bdxt_cdyt0, adz, u3, u[2], u[1], u[0]);
            u[3] = u3;
            finlength = fast_expansion_sum_zeroelim(finlength, finnow, 4, u,
                finother);
            finswap = finnow; finnow = finother; finother = finswap;
            if (adztail != 0.0) {
                ops.Two_One_Product(bdxt_cdyt1, bdxt_cdyt0, adztail, u3, u[2], u[1], u[0]);
                u[3] = u3;
                finlength = fast_expansion_sum_zeroelim(finlength, finnow, 4, u,
                    finother);
                finswap = finnow; finnow = finother; finother = finswap;
        ...
        if (adytail != 0.0) {
            negate = -bdxtail;
            ops.Two_Product(negate, adytail, bdxt_adyt1, bdxt_adyt0);
            ops.Two_One_Product(bdxt_adyt1, bdxt_adyt0, cdz, u3, u[2], u[1], u[0]);
            u[3] = u3;
            finlength = fast_expansion_sum_zeroelim(finlength, finnow, 4, u,
                finother);
            finswap = finnow; finnow = finother; finother = finswap;
            if (cdztail != 0.0) {
                ops.Two_One_Product(bdxt_adyt1, bdxt_adyt0, cdztail, u3, u[2], u[1], u[0]);
                u[3] = u3;
                finlength = fast_expansion_sum_zeroelim(finlength, finnow, 4, u,
                    finother);
                finswap = finnow; finnow = finother; finother = finswap;
    ...
    if (cdxtail != 0.0) {
        if (adytail != 0.0) {
            ops.Two_Product(cdxtail, adytail, cdxt_adyt1, cdxt_adyt0);
            ops.Two_One_Product(cdxt_adyt1, cdxt_adyt0, bdz, u3, u[2], u[1], u[0]);
            u[3] = u3;
            finlength = fast_expansion_sum_zeroelim(finlength, finnow, 4, u,
                finother);
            finswap = finnow; finnow = finother; finother = finswap;
            if (bdztail != 0.0) {
                ops.Two_One_Product(cdxt_adyt1, cdxt_adyt0, bdztail, u3, u[2], u[1], u[0]);
                u[3] = u3;
                finlength = fast_expansion_sum_zeroelim(finlength, finnow, 4, u,
                    finother);
                finswap = finnow; finnow = finother; finother = finswap;
        ...
        if (bdytail != 0.0) {
            negate = -cdxtail;
            ops.Two_Product(negate, bdytail, cdxt_bdyt1, cdxt_bdyt0);
            ops.Two_One_Product(cdxt_bdyt1, cdxt_bdyt0, adz, u3, u[2], u[1], u[0]);
            u[3] = u3;
            finlength = fast_expansion_sum_zeroelim(finlength, finnow, 4, u,
                finother);
            finswap = finnow; finnow = finother; finother = finswap;
            if (adztail != 0.0) {
                ops.Two_One_Product(cdxt_bdyt1, cdxt_bdyt0, adztail, u3, u[2], u[1], u[0]);
                u[3] = u3;
                finlength = fast_expansion_sum_zeroelim(finlength, finnow, 4, u,
                    finother);
                finswap = finnow; finnow = finother; finother = finswap;
...
double predicates::orient3d(double* pa, double* pb, double* pc, double* pd)
{
    double adx, bdx, cdx, ady, bdy, cdy, adz, bdz, cdz;
...
double incircleadapt(double* pa, double* pb, double* pc, double* pd, double permanent)
{
    ...
    if ((bdxtail != 0.0) || (bdytail != 0.0)
        ...
        ops.Square(adx, adxadx1, adxadx0);
        ops.Square(ady, adyady1, adyady0);
        ops.Two_Two_Sum(adxadx1, adxadx0, adyady1, adyady0, aa3, aa[2], aa[1], aa[0]);
        aa[3] = aa3;
    }
    if ((cdxtail != 0.0) || (cdytail != 0.0)
        || (adxtail != 0.0) || (adytail != 0.0)) {
        ops.Square(bdx, bdxbdx1, bdxbdx0);
        ops.Square(bdy, bdybdy1, bdybdy0);
        ops.Two_Two_Sum(bdxbdx1, bdxbdx0, bdybdy1, bdybdy0, bb3, bb[2], bb[1], bb[0]);
        bb[3] = bb3;
    }
    if ((adxtail != 0.0) || (adytail != 0.0)
        || (bdxtail != 0.0) || (bdytail != 0.0)) {
        ops.Square(cdx, cdxcdx1, cdxcdx0);
        ops.Square(cdy, cdycdy1, cdycdy0);
        ops.Two_Two_Sum(cdxcdx1, cdxcdx0, cdycdy1, cdycdy0, cc3, cc[2], cc[1], cc[0]);
        cc[3] = cc3;
    }
    ...
    if ((adxtail != 0.0) || (adytail != 0.0)) {
        if ((bdxtail != 0.0) || (bdytail != 0.0)
            || (cdxtail != 0.0) || (cdytail != 0.0)) {
            ops.Two_Product(bdxtail, cdy, ti1, ti0);
            ops.Two_Product(bdx, cdytail, tj1, tj0);
            ops.Two_Two_Sum(ti1, ti0, tj1, tj0, u3, u[2], u[1], u[0]);
            u[3] = u3;
            negate = -bdy;
            ops.Two_Product(cdxtail, negate, ti1, ti0);
            negate = -bdytail;
            ops.Two_Product(cdx, negate, tj1, tj0);
            ops.Two_Two_Sum(ti1, ti0, tj1, tj0, v3, v[2], v[1], v[0]);
            v[3] = v3;
            bctlen = fast_expansion_sum_zeroelim(4, u, 4, v, bct);

            ops.Two_Product(bdxtail, cdytail, ti1, ti0);
            ops.Two_Product(cdxtail, bdytail, tj1, tj0);
            ops.Two_Two_Diff(ti1, ti0, tj1, tj0, bctt3, bctt[2], bctt[1], bctt[0]);
            bctt[3] = bctt3;
            bcttlen = 4;
        }
        else {
    ...
    if ((bdxtail != 0.0) || (bdytail != 0.0)) {
        if ((cdxtail != 0.0) || (cdytail != 0.0)
            || (adxtail != 0.0) || (adytail != 0.0)) {
            ops.Two_Product(cdxtail, ady, ti1, ti0);
            ops.Two_Product(cdx, adytail, tj1, tj0);
            ops.Two_Two_Sum(ti1, ti0, tj1, tj0, u3, u[2], u[1], u[0]);
            u[3] = u3;
            negate = -cdy;
            ops.Two_Product(adxtail, negate, ti1, ti0);
            negate = -cdytail;
            ops.Two_Product(adx, negate, tj1, tj0);
            ops.Two_Two_Sum(ti1, ti0, tj1, tj0, v3, v[2], v[1], v[0]);
            v[3] = v3;
            catlen = fast_expansion_sum_zeroelim(4, u, 4, v, cat);

            ops.Two_Product(cdxtail, adytail, ti1, ti0);
            ops.Two_Product(adxtail, cdytail, tj1, tj0);
            ops.Two_Two_Diff(ti1, ti0, tj1, tj0, catt3, catt[2], catt[1], catt[0]);
            catt[3] = catt3;
            cattlen = 4;
        }
        else {
    ...
    if ((cdxtail != 0.0) || (cdytail != 0.0)) {
        if ((adxtail != 0.0) || (adytail != 0.0)
            || (bdxtail != 0.0) || (bdytail != 0.0)) {
            ops.Two_Product(adxtail, bdy, ti1, ti0);
            ops.Two_Product(adx, bdytail, tj1, tj0);
            ops.Two_Two_Sum(ti1, ti0, tj1, tj0, u3, u[2], u[1], u[0]);
            u[3] = u3;
            negate = -ady;
            ops.Two_Product(bdxtail, negate, ti1, ti0);
            negate = -adytail;
            ops.Two_Product(bdx, negate, tj1, tj0);
            ops.Two_Two_Sum(ti1, ti0, tj1, tj0, v3, v[2], v[1], v[0]);
            v[3] = v3;
            abtlen = fast_expansion_sum_zeroelim(4, u, 4, v, abt);

            ops.Two_Product(adxtail, bdytail, ti1, ti0);
            ops.Two_Product(bdxtail, adytail, tj1, tj0);
            ops.Two_Two_Diff(ti1, ti0, tj1, tj0, abtt3, abtt[2], abtt[1], abtt[0]);
            abtt[3] = abtt3;
            abttlen = 4;
        }
        else {
...
double insphereexact(double* pa, double* pb, double* pc, double* pd, double* pe)
{
    ...
    ops.Two_Product(pa[0], pb[1], axby1, axby0);
    ops.Two_Product(pb[0], pa[1], bxay1, bxay0);
    ops.Two_Two_Diff(axby1, axby0, bxay1, bxay0, ab[3], ab[2], ab[1], ab[0]);

    ops.Two_Product(pb[0], pc[1], bxcy1, bxcy0);
    ops.Two_Product(pc[0], pb[1], cxby1, cxby0);
    ops.Two_Two_Diff(bxcy1, bxcy0, cxby1, cxby0, bc[3], bc[2], bc[1], bc[0]);

    ops.Two_Product(pc[0], pd[1], cxdy1, cxdy0);
    ops.Two_Product(pd[0], pc[1], dxcy1, dxcy0);
    ops.Two_Two_Diff(cxdy1, cxdy0, dxcy1, dxcy0, cd[3], cd[2], cd[1], cd[0]);

    ops.Two_Product(pd[0], pe[1], dxey1, dxey0);
    ops.Two_Product(pe[0], pd[1], exdy1, exdy0);
    ops.Two_Two_Diff(dxey1, dxey0, exdy1, exdy0, de[3], de[2], de[1], de[0]);

    ops.Two_Product(pe[0], pa[1], exay1, exay0);
    ops.Two_Product(pa[0], pe[1], axey1, axey0);
    ops.Two_Two_Diff(exay1, exay0, axey1, axey0, ea[3], ea[2], ea[1], ea[0]);

    ops.Two_Product(pa[0], pc[1], axcy1, axcy0);
    ops.Two_Product(pc[0], pa[1], cxay1, cxay0);
    ops.Two_Two_Diff(axcy1, axcy0, cxay1, cxay0, ac[3], ac[2], ac[1], ac[0]);

    ops.Two_Product(pb[0], pd[1], bxdy1, bxdy0);
    ops.Two_Product(pd[0], pb[1], dxby1, dxby0);
    ops.Two_Two_Diff(bxdy1, bxdy0, dxby1, dxby0, bd[3], bd[2], bd[1], bd[0]);

    ops.Two_Product(pc[0], pe[1], cxey1, cxey0);
    ops.Two_Product(pe[0], pc[1], excy1, excy0);
    ops.Two_Two_Diff(cxey1, cxey0, excy1, excy0, ce[3], ce[2], ce[1], ce[0]);

    ops.Two_Product(pd[0], pa[1], dxay1, dxay0);
    ops.Two_Product(pa[0], pd[1], axdy1, axdy0);
    ops.Two_Two_Diff(dxay1, dxay0, axdy1, axdy0, da[3], da[2], da[1], da[0]);

    ops.Two_Product(pe[0], pb[1], exby1, exby0);
    ops.Two_Product(pb[0], pe[1], bxey1, bxey0);
    ops.Two_Two_Diff(exby1, exby0, bxey1, bxey0, eb[3], eb[2], eb[1], eb[0]);
...
double insphereslow(double* pa, double* pb, double* pc, double* pd, double* pe)
{
    ...
    ops.Two_Diff(pa[0], pe[0], aex, aextail);
    ops.Two_Diff(pa[1], pe[1], aey, aeytail);
    ops.Two_Diff(pa[2], pe[2], aez, aeztail);
    ops.Two_Diff(pb[0], pe[0], bex, bextail);
    ops.Two_Diff(pb[1], pe[1], bey, beytail);
    ops.Two_Diff(pb[2], pe[2], bez, beztail);
    ops.Two_Diff(pc[0], pe[0], cex, cextail);
    ops.Two_Diff(pc[1], pe[1], cey, ceytail);
    ops.Two_Diff(pc[2], pe[2], cez, ceztail);
    ops.Two_Diff(pd[0], pe[0], dex, dextail);
    ops.Two_Diff(pd[1], pe[1], dey, deytail);
    ops.Two_Diff(pd[2], pe[2], dez, deztail);

    ops.Two_Two_Product(aex, aextail, bey, beytail,
        axby7, axby[6], axby[5], axby[4],
        axby[3], axby[2], axby[1], axby[0]);
    axby[7] = axby7;
    negate = -aey;
    negatetail = -aeytail;
    ops.Two_Two_Product(bex, bextail, negate, negatetail,
        bxay7, bxay[6], bxay[5], bxay[4],
        bxay[3], bxay[2], bxay[1], bxay[0]);
    bxay[7] = bxay7;
    ablen = fast_expansion_sum_zeroelim(8, axby, 8, bxay, ab);
    ops.Two_Two_Product(bex, bextail, cey, ceytail,
        bxcy7, bxcy[6], bxcy[5], bxcy[4],
        bxcy[3], bxcy[2], bxcy[1], bxcy[0]);
    bxcy[7] = bxcy7;
    negate = -bey;
    negatetail = -beytail;
    ops.Two_Two_Product(cex, cextail, negate, negatetail,
        cxby7, cxby[6], cxby[5], cxby[4],
        cxby[3], cxby[2], cxby[1], cxby[0]);
    cxby[7] = cxby7;
    bclen = fast_expansion_sum_zeroelim(8, bxcy, 8, cxby, bc);
    ops.Two_Two_Product(cex, cextail, dey, deytail,
        cxdy7, cxdy[6], cxdy[5], cxdy[4],
        cxdy[3], cxdy[2], cxdy[1], cxdy[0]);
    cxdy[7] = cxdy7;
    negate = -cey;
    negatetail = -ceytail;
    ops.Two_Two_Product(dex, dextail, negate, negatetail,
        dxcy7, dxcy[6], dxcy[5], dxcy[4],
        dxcy[3], dxcy[2], dxcy[1], dxcy[0]);
    dxcy[7] = dxcy7;
    cdlen = fast_expansion_sum_zeroelim(8, cxdy, 8, dxcy, cd);
    ops.Two_Two_Product(dex, dextail, aey, aeytail,
        dxay7, dxay[6], dxay[5], dxay[4],
        dxay[3], dxay[2], dxay[1], dxay[0]);
    dxay[7] = dxay7;
    negate = -dey;
    negatetail = -deytail;
    ops.Two_Two_Product(aex, aextail, negate, negatetail,
        axdy7, axdy[6], axdy[5], axdy[4],
        axdy[3], axdy[2], axdy[1], axdy[0]);
    axdy[7] = axdy7;
    dalen = fast_expansion_sum_zeroelim(8, dxay, 8, axdy, da);
    ops.Two_Two_Product(aex, aextail, cey, ceytail,
    ...
    ops.Two_Two_Product(bex, bextail, dey, deytail,
...
double insphereadapt(double* pa, double* pb, double* pc, double* pd, double* pe, double permanent)
{
    ...
    ops.Two_Product(aex, bey, aexbey1, aexbey0);
    ops.Two_Product(bex, aey, bexaey1, bexaey0);
    ops.Two_Two_Diff(aexbey1, aexbey0, bexaey1, bexaey0, ab3, ab[2], ab[1], ab[0]);
    ab[3] = ab3;

    ops.Two_Product(bex, cey, bexcey1, bexcey0);
    ops.Two_Product(cex, bey, cexbey1, cexbey0);
    ops.Two_Two_Diff(bexcey1, bexcey0, cexbey1, cexbey0, bc3, bc[2], bc[1], bc[0]);
    bc[3] = bc3;

    ops.Two_Product(cex, dey, cexdey1, cexdey0);
    ops.Two_Product(dex, cey, dexcey1, dexcey0);
    ops.Two_Two_Diff(cexdey1, cexdey0, dexcey1, dexcey0, cd3, cd[2], cd[1], cd[0]);
    cd[3] = cd3;

    ops.Two_Product(dex, aey, dexaey1, dexaey0);
    ops.Two_Product(aex, dey, aexdey1, aexdey0);
    ops.Two_Two_Diff(dexaey1, dexaey0, aexdey1, aexdey0, da3, da[2], da[1], da[0]);
    da[3] = da3;

    ops.Two_Product(aex, cey, aexcey1, aexcey0);
    ops.Two_Product(cex, aey, cexaey1, cexaey0);
    ops.Two_Two_Diff(aexcey1, aexcey0, cexaey1, cexaey0, ac3, ac[2], ac[1], ac[0]);
    ac[3] = ac3;

    ops.Two_Product(bex, dey, bexdey1, bexdey0);
    ops.Two_Product(dex, bey, dexbey1, dexbey0);
    ops.Two_Two_Diff(bexdey1, bexdey0, dexbey1, dexbey0, bd3, bd[2], bd[1], bd[0]);
    bd[3] = bd3;
   ...
   ops.Two_Diff_Tail(pa[0], pe[0], aex, aextail);
   ops.Two_Diff_Tail(pa[1], pe[1], aey, aeytail);
   ops.Two_Diff_Tail(pa[2], pe[2], aez, aeztail);
   ops.Two_Diff_Tail(pb[0], pe[0], bex, bextail);
   ops.Two_Diff_Tail(pb[1], pe[1], bey, beytail);
   ops.Two_Diff_Tail(pb[2], pe[2], bez, beztail);
   ops.Two_Diff_Tail(pc[0], pe[0], cex, cextail);
   ops.Two_Diff_Tail(pc[1], pe[1], cey, ceytail);
   ops.Two_Diff_Tail(pc[2], pe[2], cez, ceztail);
   ops.Two_Diff_Tail(pd[0], pe[0], dex, dextail);
   ops.Two_Diff_Tail(pd[1], pe[1], dey, deytail);
   ops.Two_Diff_Tail(pd[2], pe[2], dez, deztail);
