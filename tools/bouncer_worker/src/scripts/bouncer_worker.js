...
async function handleMessage(cmd, rid, callback){
	...
	if(cmd.startsWith('importToy')){
		...
		try {
			await importToyModel(dir, database, model, skipPostProcessing);
...
async function runBouncer(logDir, cmd,  callback)
{
	...
	try {
		...
		if (command === "import") {
			...
			await importToy(dir, cmdDatabase, cmdProject, {tree: 1});
		}
		callback({
			value: code,
			database: cmdDatabase,
...
async function exeCommand(cmd, rid, callback){
	const logRootDir = config.bouncer.log_dir;
	const logDir = logRootDir + "/" +  rid.toString() + "/";
...
const startBouncerWorker = async () => {
	...
	await testClient();
	connectToQueue(handleMessage, handleMessage);
}
